 ...
 *
 * Riemann solvers compute fluxes by solving the Riemann problem
 * at each cell interface.
 */
class RiemannSolverFlux : public FluxCalculator {
public:
    explicit RiemannSolverFlux(const std::string& name)
        : FluxCalculator(name, "riemann") {}

    /**
     * Most Riemann solvers need wave speed for CFL condition.
     */
    bool needs_wave_speed() const override { return true; }
};

/**
 * Base class for dissipative flux calculators.
 *
 * Dissipative methods add artificial viscosity for stability.
 * Examples: Lax-Friedrichs, Rusanov.
 */
class DissipativeFluxCalculator : public FluxCalculator {
public:
    explicit DissipativeFluxCalculator(const std::string& name)
        : FluxCalculator(name, "dissipative") {}

    /**
     * Dissipative methods always need wave speed for diffusion term.
     */
    bool needs_wave_speed() const override { return true; }
};
 ...
 *
 * Central schemes compute fluxes using central differencing
 * without explicit Riemann problem solving.
 */
class CentralFluxCalculator : public FluxCalculator {
public:
    explicit CentralFluxCalculator(const std::string& name)
        : FluxCalculator(name, "central") {}
};
